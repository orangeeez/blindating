...
export class BaseService {
    ...
    public GetLast(userID: number): Observable<any> {
        return this._authHttp.post(API_ADDRESS + this.api + "/getlast", JSON.stringify(userID), this.options)
            .map(user => user.json()['result']);
    }
    public Add(entity: any): Observable<number> {
        return this._authHttp.post(API_ADDRESS + this.api + "/add", JSON.stringify(entity), this.options)
            .map(user => user.json()['result']);
    }
    ...
    public Remove(entity: any): Observable<boolean> {
        return this._authHttp.post(API_ADDRESS + this.api + "/remove", JSON.stringify(entity), this.options)
            .map(user => user.json()['result']);
    }
}